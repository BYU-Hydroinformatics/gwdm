...
def get_thredds_value(server, layer, bbox):
    ...
    server = f"{server}&LAYERS={layer}&STYLES=raster/default&TIME={time_min}/{time_max}"
    print(server)
...
def sat_resample(gldas_df):
    ...
    # gldas_df = gldas_df.set_index("time")
...
def impute_data(comb_df, well_names, names):
    ...
    for well in well_names:  # list of the wells in the aquifer
        ...
        W_out = np.linalg.lstsq(
            X.T.dot(X) + lamb_value * identity, X.T.dot(ty), rcond=-1
        )[0]
